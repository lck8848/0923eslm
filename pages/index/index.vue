<template>
	<view class="container">
		...
		<view class="home-container">
			<!-- 搜索 -->
			<navigator url="/pages/search/search" hover-class="none"><van-search value="" class="search" background="#008EFF" placeholder="搜索饿了么商家 , 商品名称" /></navigator>
			...
			<view class="quality">
				<view class="top">
					...
					<view class="right"><image class="img" src="/static/images/home/food.webp"></image></view>
				</view>
				<view class="footer">
					<van-cell value="限时6元开通" is-link value-class="red">
						<view slot="title">
							<view class="van-cell-text">
								...
								<text class="left">超级会员</text>
								·
								<text class="center">每月领20元红包</text>
							</view>
						</view>
					</van-cell>
				</view>
			...
			<!-- 分割线 -->
			<van-divider contentPosition="center" customStyle="color: #000; border-color: #eee; font-size: 32rpx;">为你推荐</van-divider>
			...
			<view class="merchant-container">
				<view class="item" v-for="item in merchantList" :key="item.id">
					<view class="left"><image class="food_img" :src="item.m_img" mode=""></image></view>
					<view class="right">
						<view class="title">{{ item.m_name }}</view>
						<view class="sales">
							<uni-rate size="13" class="rateAbsu" allow-half :value="item.grade" />
							{{ item.grade }} 月售{{ item.sales_number }}单
						</view>
						<view class="footer">
							<view class="distribution">￥{{ item.start_money }}起送 | {{ getDistribution(item.distribution) }}</view>
							<view class="distance">{{ getDistance(item.distance) }} | {{ item.time }}分钟</view>
						</view>
						<view class="specialty">{{ item.signboard }}</view>
					</view>
				</view>
			</view>
		</view>
		<view class="filex" v-show="isMenuFix" @click="getTop">
			<image
				class="filexImg"
